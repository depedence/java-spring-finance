...
export function useCreateTransaction() {
  ...
  return useMutation(
    (payload: Transaction) => api.post<Transaction>("/api/transactions", payload).then(r => r.data),
    {
      onMutate: async (newTx) => {
        await qc.cancelQueries([KEY]);
        const previous = qc.getQueryData<Transaction[]>([KEY]) ?? [];
        qc.setQueryData([KEY], [...previous, { ...newTx, id: Math.random() }]); // optimistic temp id
...
export function useUpdateTransaction() {
  const qc = useQueryClient();
  return useMutation(
    ({ id, payload }: { id: number; payload: Transaction }) => api.put<Transaction>(`/api/transactions/${id}`, payload).then(r => r.data),
    {
...
export function useDeleteTransaction() {
  const qc = useQueryClient();
  return useMutation(
    (id: number) => api.delete(`/api/transactions/${id}`).then(r => r.data),
    {
